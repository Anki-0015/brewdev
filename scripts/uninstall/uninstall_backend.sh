...
# Function to uninstall backend tools
uninstall_backend_tools() {
    echo -e "${BLUE}Uninstalling backend tools...${NC}"
...
# Show help
show_help() {
    echo -e "${BLUE}Backend Environment Uninstaller${NC}"
    echo
    echo -e "${YELLOW}Usage:${NC} ./uninstall_backend.sh [language]"
    echo
    ...
    echo -e "  ./uninstall_backend.sh python"
    echo -e "  ./uninstall_backend.sh node"
}
...
echo -e "${YELLOW}Are you sure you want to uninstall the backend environment with $1? (y/N)${NC}"
read -r response
...
echo -e "${BLUE}Starting backend environment uninstallation...${NC}"
